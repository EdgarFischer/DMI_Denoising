...
CONFIG_PATH="${PROJECT_ROOT}/configs/train_physics_7T_phive_simulation_snr9.yaml"
RUN_NAME="Simulation_SNR9_PHIVE"
...
for required_file in "${CONFIG_PATH}" "${DATA_FILE}" "${MASK_FILE}"; do
...
{
    echo "[$(date --iso-8601=seconds)] Starting PhysicsConv3D PHIVE on SNR-9 simulation"
    ...
    echo "Normalization: disabled; preserve Simulation_GT global scale"
    echo "Ground truth: never exposed during training"
    echo
} > "${LOG_FILE}"
...
if kill -0 "${PID}" 2>/dev/null; then
    echo "Simulation PHIVE training started with PID ${PID}"
    echo "Log: ${LOG_FILE}"
