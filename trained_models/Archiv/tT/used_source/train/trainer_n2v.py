...
def train_n2v():
    ...
    # ─── DEBUG: Masken-Check ─────────────────────────────────────────────────
    inp, tgt, mask = next(iter(train_loader))
    # mask hat Shape (B,1,H,W)
    print(f"[DEBUG] mask.shape = {mask.shape}, dtype = {mask.dtype}")
    sums = mask.view(mask.size(0), -1).sum(dim=1).tolist()
    print(f"[DEBUG] mask.sum() pro Bild im ersten Batch = {sums}")
