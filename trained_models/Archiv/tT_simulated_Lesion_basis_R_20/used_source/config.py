...
# ---------------------------------------------------------------------------
UNET_DIM     = "2d"                 #  2d  | 3d
TRAIN_METHOD = "n2v_basis"                #  n2v = unet | p2n (auch unet) | ynet_n2v (noise 2 void) | sup_lowrank, n2v_basis: Basis-basierter Denoiser

lowrank_rank      = 20
basis_file        = f"datasets/basis_rank{lowrank_rank}.npy"   # ← Pfad zur V_r.npy
...
_TRAINER_MAP = {
    ...
    ("n2v_basis","2d"): ("train.trainer_unet_basis",      "train_unet_basis"),
}
...
# ---------------------------------------------------------------------------
GPU_NUMBER = "0"
RUN_NAME   = "tT_simulated_Lesion_basis_R_20"
...
fourier_transform_axes = []      # FFT über FID-Achse (t)
fixed_indices   = None
num_samples = 10000
val_samples = 2000
