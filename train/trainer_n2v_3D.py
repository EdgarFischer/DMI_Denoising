...
import config                                           # ← passe hier 3-D-Parameter an
...
def prepare_dataset(folders: List[str], transform, num_samples: int):
    ...
    return MRSiN3DDataset(
        data          = data,
        volume_axes   = config.volume_axes,   # z. B. (0,1,2)
...
def train_n2v_3d():
    ...
    # ─── Debug: Maske checken ──────────────────────────────────────────────
    inp_dbg, tgt_dbg, mask_dbg = next(iter(train_loader))
    print(f"[DEBUG] mask.shape = {mask_dbg.shape}")          # (B,1,D,H,W)
    print(f"[DEBUG] voxels/Volumen = {mask_dbg[0].sum().item()}")
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        with torch.no_grad():
            for inp, tgt, mask in val_loader:
                inp, tgt, mask = inp.to(device), tgt.to(device), mask.to(device)
                running += masked_mse_loss(model(inp), tgt, mask).item() * inp.size(0)
